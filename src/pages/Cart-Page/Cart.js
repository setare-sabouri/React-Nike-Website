...
const Cart = ({ carts, handleDelete }) => {
  ...
  return (
    <div>
      ...
      {carts.length === 0 && <div className="empty-cart">Your cart is currently empty</div>}
      ...
      {carts.length >= 1 && <div className="total-price">{`Your total is:  $${totalPrice}`} <br /> <button className="checkout-btn">Proceed to checkout</button> </div>}
    </div>
  );
};
export default Cart;
